...
|api.default.consumes |String |Comma separated list of acceptable request body content types when `RestParamType.body` is used without explicitly defining content types via `.consumes()`.

|api.default.produces |String |Comma separated list of default content types when `RestParamType.body` is used without explicitly defining content types via `.consumes()`.
|===
