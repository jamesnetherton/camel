...
|api.default.consumes |String |Comma separated list of default media types when `RestParamType.body` is used without providing a custom `.consumes()` configuration. The default value is `application/json`.

|api.default.produces |String |Comma separated list of default media types when `outType` is used without providing a custom `.produces()` configuration. The default value is `application/json`.
|===
